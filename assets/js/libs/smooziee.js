 ...
 if (typeof module === "object" && typeof module.exports === "object") {
  ...
  $.smooziee = function(options){
    ...
    var scroll = function(str){
      ...
      if(target.length !== 0){
		var pt = parseInt(target.css('padding-top'));
		...
		if($window.width() > 736){
			...
			if(hash == "#about"){
				var position = target.offset().top - headerHeight + 45;
			}else{
				var position = target.offset().top - headerHeight;
			}
		}
    ...
    $('[href*="#"]:not(' + options.ignore + ')').on('click', function(event) {
		TRINITY.MENU_HEADER.hideContent();
      var $anchor = $(this);
